...
local StatuslineHint = require("window-picker.hints.statusline")
  ...
  self.opt_cap = {
    statusline = {
      g = {
        ...
        { name = "cmdheight", value = 1 },
      },
      w = { { name = "statusline" } },
    },
    winbar = { g = {}, w = { { name = "winbar" } } },
  }
...
function M:draw(windows)
  ...
  for index, window in ipairs(windows) do
    ...
    local display_text = self.selection_display and self.selection_display(char, window) or "%=" .. char .. "%="

    local ok, result = pcall(vim.api.nvim_win_set_option, window, hint_type, display_text)
...
function M:set_plugin_hl()
  M.create_hl(self.statusline_hl_ns, HL.st_hi, self.highlights.statusline.focused)
  M.create_hl(self.statusline_hl_ns, HL.st_hi_nc, self.highlights.statusline.unfocused)

  M.create_hl(self.winbar_hl_ns, HL.wb_hi, self.highlights.winbar.focused)

  M.create_hl(self.winbar_hl_ns, HL.wb_hi_nc, self.highlights.winbar.unfocused)
end
