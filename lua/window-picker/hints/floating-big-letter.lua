...
function M:set_config(config)
  ...
  local font = config.floating_big_letter.font

  if type(font) == "table" then
    self.big_chars = font
  elseif type(font) == "string" then
    if font ~= "ansi-shadow" then
      vim.notify("Only font ansi-shadow is currently supported", vim.log.levels.WARN)
    end
    self.big_chars = require(("window-picker.hints.data.ansi-shadow"):format(font))
  end
end
